...
volatile float mygettemp;       //用于存放温度结果
...
volatile float angle[3] = {0.0f};

volatile Euler_angles_t euler_angle;
...
{  
    ...
    int i;
	//读取加速度和陀螺仪的当前ADC
    MPU6500_getMotion6(&accgyroval[0], &accgyroval[1], &accgyroval[2], &accgyroval[3], &accgyroval[4], &accgyroval[5],&accgyroval[6]);
    ...
    MPU6500_Raw_Data.Temp = accgyroval[6];    
    ...
    for(i = 0; i<6; i++) 
    {
      ...
      {
        values[i] = ((float) accgyroval[i]) / 32.768f; //转成度每秒
		//这里已经将量程改成了 1000度每秒  32.768 对应 1度每秒
      }
    }
	
    IST8310_mgetValues(&values[6]);	//读取磁力计的ADC值
    MPU6500_Raw_Data.Mag_X = values[6];
    MPU6500_Raw_Data.Mag_Y = values[7];
    MPU6500_Raw_Data.Mag_Z = values[8];
    
    //温度
    MPU6500_Raw_Data.Temp = mygettemp = accgyroval[6];
}
...
#define Ki 0.018f   // integral gain governs rate of convergence of gyroscope biases
void IMU_AHRSupdate(void) 
{
    ...
    if(now<lastUpdate)
    {
		halfT =  ((float)(now + (1000- lastUpdate)) / 2000000.0f);   //  uint 0.5s
    }
    else	
    {
    ...
    lastUpdate = now;	//更新时间
    //把加计和磁力计的三维向量转成单位向量。
    norm = invSqrt(ax*ax + ay*ay + az*az);    //快速求平方根算法   
    ax = ax * norm;
    ay = ay * norm;
    az = az * norm;    
...
volatile float yaw_temp,last_yaw_temp;
void GetPitchYawGxGyGz()
{

	
	MPU6500_Real_Data.Gyro_X = mygetqval[3] - (-0.8f);
	MPU6500_Real_Data.Gyro_Y = -mygetqval[4];
	MPU6500_Real_Data.Gyro_Z = mygetqval[5] - (0.4f);
	MPU6500_Real_Data.Accel_X = mygetqval[0]/4.096f;//mg
	MPU6500_Real_Data.Accel_Y = mygetqval[1]/4.096f;
	MPU6500_Real_Data.Accel_Z = mygetqval[2]/4.096f;
	...
	euler_angle.yaw_angle = -(yaw_temp + yaw_count*360);  //yaw轴角度

	euler_angle.pitch_angle = angle[1];
    euler_angle.roll_angle = angle[2];	
}
...
{
	while(MPU6500_Init());
	while(MPU6500_Enable());
	while(BSP_IST8310_Init());
}
